...
class LLMConfig(ABC):
    ...
    def validate_config(config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        for key in ["api_key", "github_token", "groq_api_key", "openai_api_key"]:
            if key in config:
                logger.warning(f"Removing sensitive key '{key}' from configuration")
                del config[key]

        logger.debug(f"Configuration validated with params: {list(config.keys())}")
        return config
...
class OpenAIConfig(LLMConfig):
    ...
    def __init__(
        ...
        **kwargs
        ...
        logger.info(f"Using OpenAI model: {self.model_name} with temperature: {self.temperature}")
        ...
        self.kwargs = kwargs
        logger.debug(f"Additional OpenAI parameters: {list(self.kwargs.keys())}")

    def create_llm(self) -> ChatOpenAI:
        """Create and return an OpenAI LLM instance."""
        logger.info(f"Creating OpenAI ChatLLM instance with model: {self.model_name}")
        try:
            llm = ChatOpenAI(
...
class GitHubConfig(LLMConfig):
    ...
    def __init__(
        ...
        **kwargs
        ...
        logger.info("Initializing GitHub LLM configuration")
        ...
        if not self.model:
            logger.error("GitHub model name not provided in parameters or environment")
